...
| **Avatar pose on the headset — `XsensUDPReceiver.cs`** | Sari (2026-07-16) | ✅ **VERIFIED ON DEVICE** — APK + `animate_to_unity.m` → the avatar animates on the Quest, and the canopy lines/slider render correctly. **Root cause was proven, not guessed: the XSens plugin CANNOT work in a build.** In `com.unity.live-capture`'s `ConnectionManager.cs` the only call that loads saved connections — `InternalEditorUtility.LoadSerializedFileAndForget(k_AssetPath)` — is wrapped in `#if UNITY_EDITOR`; a player build compiles it out and constructs an **empty** manager, so nothing ever binds 9763. Connections also live in `UserSettings/`, never shipped. Live Capture drives the *Editor*, not players. **Fix:** `XsensUDPReceiver.cs` on the **`XsensUDP`** object, **Mode = BuildsOnly** (plugin owns the Editor, socket owns the APK — **never both**, same bones + same port). Ported from AR's on-device-verified MXTP02 parser with two deliberate departures: **rotation-only** (`applyHipsHeight` OFF — SimulatorReceiver owns world position in VR) and **`FindSkinnedBone`-style resolution** (the `" 1"` suffix trap — a plain lookup binds the flat non-moving copies and animates nothing while receiving packets perfectly). |
