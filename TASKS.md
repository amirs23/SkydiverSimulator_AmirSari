...
| Matlab-driven movement — UDP receiver (connection working) | Sari (2026-06-17) | `SimulatorReceiver.cs` (port 9764, separate from XSens 9763) drives canopy + skydiver Rigidbodies from the Matlab stream + feeds steering → `ToggleArmAnimation` shoulder pitch; disables `PlayerMovement`. Added `PlayerMovement.SetToggles()`. Test sender `Matlab/sim_to_unity.m` (spiral descent). Connection confirmed live. **Still needs:** coordinate-frame/rotation tuning (`frameMode`/`ConvRot` — confirm real lab frame), resolve arm ownership when XSens also connected, verify against real EOM_Solver on lab Windows PC. See HANDOFF. |
