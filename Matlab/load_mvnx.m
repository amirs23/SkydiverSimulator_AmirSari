...
fclose(fid);
content = content(content ~= 13);  % strip \r so Windows line endings don't corrupt tag names
...
for n=1:length(kindofendwords)
    idx = find(strcmp(word,kindofendwords(n)),1);
    if isempty(idx), continue; end
    placeoffirststartword(n) = idx;
    placeofallstartwords{n} = find(strcmp(word,kindofendwords(n)));
    placeofallendwords{n} = find(strcmp(word,['/' kindofendwords{n}]));
end
% Remove entries with no matching start word (end-only tags in the XML)
valid = placeoffirststartword > 0;
kindofendwords      = kindofendwords(valid);
placeoffirststartword = placeoffirststartword(valid);
placeofallstartwords  = placeofallstartwords(valid);
placeofallendwords    = placeofallendwords(valid);
